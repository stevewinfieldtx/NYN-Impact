...
export default function SignInPage() {
  ...
  async function handleSignIn(e: React.FormEvent) {
    ...
    try {
      ...
      sessionStorage.setItem('nyn_auth', JSON.stringify({
        name: data.customer.name,
        id: data.customer.id,
        projects: data.projects || [],
      }));

      // If only one project, go directly to it
      if (data.projects && data.projects.length === 1) {
        sessionStorage.setItem('nyn_v_' + data.projects[0].slug, JSON.stringify({
          name: data.customer.name,
          id: data.customer.id,
        }));
        router.push('/cus/' + data.projects[0].slug);
      } else {
        // Multiple projects — go to dashboard
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-[#f0ede6] flex flex-col">
      <nav className="border-b border-white/5 bg-[#0a0a0f]/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className="text-xl font-semibold tracking-tight" style={{ fontFamily: 'Fraunces, serif' }}>
            NYN<span className="text-purple-400">Impact</span>
          </Link>
          <Link href="/" className="flex items-center gap-1.5 text-sm text-[#707080] hover:text-[#f0ede6] transition-colors">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-bold mb-3" style={{ fontFamily: 'Fraunces, serif' }}>Welcome back</h1>
            <p className="text-[#a0a0b0]">Sign in to manage your website.</p>
          </div>
          ...
          <p className="text-center text-[#505060] text-xs mt-8">
            Need help? <a href="mailto:support@nynimpact.com" className="text-purple-400 hover:text-purple-300 transition-colors">support@nynimpact.com</a>
          </p>
        </div>
      </div>
    </div>
