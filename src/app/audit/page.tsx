...
type BenchComp = { benchmark_name: string; benchmark_url: string; what_they_do_better: string[]; what_you_do_well: string[]; lesson: string; verdict: string };
...
export default function AuditPage() {
  ...
  const cn = report?.company_name || 'Your Business';
      ...
      <div className="bg-gradient-to-br from-[#1B2A4A] to-[#0F172A] px-6 pt-7 pb-6">
        <div className="max-w-[700px] mx-auto">
          ...
          <h1 className="text-2xl font-extrabold text-white mt-1.5 tracking-tight">
            {step === 4 && report ? `Revenue Audit: ${cn}` : 'Free Website Revenue Audit'}
          </h1>
          <p className="text-sm text-slate-400 mt-0.5">
            {step === 4 ? `Here's what your site is costing you, ${cn}.` : 'See exactly where your website is leaving money on the table.'}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-[700px] mx-auto px-6 mt-4 mb-16">
        ...
        {step === 1 && (
          <div className="bg-[#111118] rounded-2xl border border-white/10 p-6">
            <h2 className="text-lg font-bold text-white mb-1">Let's audit your website</h2>
            <p className="text-sm text-slate-400 mb-4">Paste your URL below. We'll show you exactly what's costing you customers — and how to fix it.</p>
        ...
        {step === 2 && (
          <div className="bg-[#111118] rounded-2xl border border-white/10 p-6">
            <h2 className="text-lg font-bold text-white mb-1">Tell us about your customers</h2>
            <p className="text-sm text-slate-400 mb-5">The more we know about who buys from you, the sharper we can pinpoint where you're losing them. Not sure? Hit <span className="font-semibold text-teal-400">Auto-Detect</span> and we'll figure it out.</p>

            <div className="mb-5">
              <label className="text-sm font-semibold text-slate-300 block mb-1.5">Your industry <span className="font-normal text-slate-500">(optional)</span></label>
              <input value={industry} onChange={e => setIndustry(e.target.value)} placeholder="e.g. French bakery, managed IT, dental practice, SaaS"
            ...
            <div className="mb-5">
              <div className="flex justify-between items-center mb-1.5">
                <label className="text-sm font-semibold text-slate-300">Who is your ideal customer?</label>
              ...
              {pLoading && <div className="flex items-center gap-2 p-3 bg-teal-500/10 rounded-lg text-sm text-teal-400 mb-2">
                <div className="w-3 h-3 border-2 border-teal-400 border-t-transparent rounded-full animate-spin" /> Reading your site to understand your buyer...</div>}
              <textarea value={persona} onChange={e => { setPersona(e.target.value); setPAuto(false); }} rows={4}
                placeholder="Describe the person who pays you: their role, what they care about, what makes them choose you (or not). Or hit Auto-Detect."
              ...
              {pAuto && persona && !pLoading && <p className="text-xs text-teal-400 mt-1">✓ Auto-generated from your site — edit if anything's off.</p>}
              <p className="text-xs text-slate-500 mt-1">This is how we evaluate whether your site would convince a real buyer to pick up the phone.</p>
            </div>

            <div className="mb-5">
              <div className="flex justify-between items-center mb-2">
                <div>
                  <label className="text-sm font-semibold text-slate-300 block">Best-in-Class Examples</label>
                  <p className="text-xs text-slate-500 mt-0.5">The gold standard in your industry — websites or social pages you should be measured against.</p>
                </div>
              ...
              {bLoading && <div className="flex items-center gap-2 p-3 bg-teal-500/10 rounded-lg text-sm text-teal-400 mb-2">
                <div className="w-3 h-3 border-2 border-teal-400 border-t-transparent rounded-full animate-spin" /> Finding the best-in-class examples in your industry...</div>}
              {[0, 1, 2].map(i => (
                <div key={i} className="flex items-center gap-2 mb-2">
                  ...
                  <input value={benchmarks[i]} onChange={e => setB(i, e.target.value)}
                    placeholder={['https://best-example.com or facebook.com/best-page', 'https://another-great-one.com', 'https://industry-leader.com (optional)'][i]}
                    className="w-full px-3.5 py-2.5 text-sm text-white bg-[#0a0a0f] rounded-lg border-2 border-slate-600 focus:border-teal-500 outline-none transition placeholder:text-slate-500" />
                </div>
              ))}
              {bAuto && benchmarks[0] && !bLoading && <p className="text-xs text-teal-400 mt-1">✓ Found the top examples via AI. Edit or replace any that aren't right.</p>}
            </div>
            ...
            <div className="flex justify-between pt-4 border-t border-white/10">
              ...
              <button onClick={runAudit} className="px-7 py-2.5 text-sm font-bold rounded-lg bg-teal-600 text-white hover:bg-teal-500 transition">Run Revenue Audit →</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="bg-[#111118] rounded-2xl border border-white/10 p-8 text-center">
            ...
            <h2 className="text-lg font-bold text-white mb-2">Analyzing your revenue impact...</h2>
            <p className="text-sm text-slate-400 mb-4">We're looking at your site through your buyer's eyes, comparing you against the best in your industry.</p>
          ...
          <div ref={ref} className="space-y-4">
              ...
              <div className="p-5 border-b border-white/10">
                <div className="flex justify-between items-start flex-wrap gap-3">
                  <div className="flex-1 min-w-[200px]">
                    <p className="text-[11px] font-bold text-teal-400 uppercase tracking-wider mb-1">Revenue Audit For</p>
            ...
            {report.trust_ratio && (
              <div className="bg-[#111118] rounded-2xl border border-white/10 p-4 flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">{report.trust_ratio.supported_claims} of {report.trust_ratio.total_claims} claims on your site are backed by proof.</p>
            ...
            {report.revenue_opportunity && (
              <div className="bg-[#111118] rounded-2xl border border-white/10 overflow-hidden border-l-4 border-l-green-500">
                <div className="p-5">
                  <h3 className="text-[15px] font-bold text-white mb-3">📈 Your Revenue Opportunity</h3>
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="p-3 bg-red-500/10 rounded-lg">
                      <p className="text-[11px] font-bold text-red-400 mb-1">YOUR SITE TODAY</p>
                      <p className="text-xs text-slate-300 leading-relaxed">{report.revenue_opportunity.current_state}</p>
                    </div>
                    <div className="p-3 bg-green-500/10 rounded-lg">
                      <p className="text-[11px] font-bold text-green-400 mb-1">YOUR SITE AFTER FIXES</p>
            ...
            {report.top_5_fixes && (
              <div className="bg-[#111118] rounded-2xl border border-white/10 overflow-hidden">
                <div className="px-5 py-3 bg-white/5"><h3 className="text-sm font-bold text-white">🔧 What You Should Fix First</h3></div>
            ...
            {report.benchmark_comparisons?.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-base font-extrabold text-white">How You Compare to the Best</h3>
                </div>
                  ...
                  return (
                    <div key={i} className={`bg-[#111118] rounded-2xl border border-white/10 overflow-hidden mb-3 border-l-4 ${accents[i % 3]}`}>
                      <div className="px-4 py-2.5 bg-white/5">
                        <h4 className="text-sm font-bold text-white">You vs. {bench.benchmark_name}</h4>
                        <p className="text-[10px] text-slate-500">{bench.benchmark_url}</p>
                      </div>
                      <div className="p-4">
                        {bench.what_they_do_better?.length > 0 && (
                          <div className="mb-2.5">
                            <p className="text-[11px] font-bold text-red-400 mb-1">✗ What They Do That You Don't</p>
                        ...
                        {bench.what_you_do_well?.length > 0 && (
                          <div className="mb-2.5">
                            <p className="text-[11px] font-bold text-green-400 mb-1">✓ Where You Already Shine</p>
                            {bench.what_you_do_well.map((w: string, j: number) => <p key={j} className="text-xs text-slate-400 leading-relaxed pl-2.5 mb-0.5">• {w}</p>)}
                          </div>
                        )}
                        {bench.lesson && <p className="text-[11px] mb-1.5"><span className="font-bold text-amber-400">💡 Steal this:</span> <span className="text-slate-300">{bench.lesson}</span></p>}
                ...
                {report.benchmark_summary && report.benchmark_summary !== 'N/A' && (
                  <div className="bg-[#111118] rounded-2xl border border-white/10 p-4 border-l-4 border-l-amber-500">
                    <p className="text-xs font-bold text-white mb-1">🎯 The Bottom Line</p>
            ...
            {report.gap_analysis && (
              <div className="bg-[#111118] rounded-2xl border border-white/10 overflow-hidden">
                <div className="px-5 py-3 bg-gradient-to-r from-teal-600 to-[#1B2A4A]">
                  <h3 className="text-sm font-bold text-white">🕳️ What Your Buyers Need But Don't Get</h3>
                </div>
                <div className="p-5 space-y-3.5">
                  {report.gap_analysis.unanswered_questions?.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-slate-200 mb-1.5">Questions Your Buyers Have That You Don't Answer</p>
                  ...
                  {report.gap_analysis.unaddressed_objections?.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-slate-200 mb-1.5">Objections That Are Sending Your Buyers Elsewhere</p>
                  ...
                  {report.gap_analysis.missing_evidence?.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-slate-200 mb-1.5">Claims You Make Without Any Proof</p>
            ...
            <div className="bg-gradient-to-br from-[#1B2A4A] to-[#0F172A] rounded-2xl p-7 text-center border border-white/10">
              <h3 className="text-lg font-extrabold text-white mb-1.5">Ready to fix this?</h3>
              <p className="text-sm text-slate-400 leading-relaxed max-w-md mx-auto mb-5">
                NYN Impact builds websites that answer your buyer's questions, outperform the best in your industry, and convert visitors into paying customers. Every fix above is something we can implement for you.
              </p>
              ...
              <p className="text-[11px] text-slate-600 mt-4">Powered by NYN Impact · nynimpact.com</p>
            </div>
          </div>
        )}
      </div>
